...
НЕ ИЗВЛИЧАЙ:
- Дрехи (тениски, потници, екипи за бягане)
- Аксесоари и чанти
- Козметика
- Всичко, което НЕ Е храна

ИНСТРУКЦИИ:
1. Намери само ХРАНИТЕЛНИ продукти на Harmonica/Хармоника
2. За всеки продукт извлечи ТОЧНОТО име както е написано
3. Извлечи цената в лева (BGN) - число с 2 десетични знака
...
Ако не намериш ХРАНИТЕЛНИ продукти на Harmonica: []"""
...
НАШИЯТ СПИСЪК (номер, име, грамаж):
{our_products_text}

ПРОДУКТИ НАМЕРЕНИ В САЙТА (с цени):
{found_products_text}

СТРИКТНИ ПРАВИЛА ЗА СЪПОСТАВЯНЕ:

1. ГРАМАЖЪТ/ОБЕМЪТ Е ЗАДЪЛЖИТЕЛЕН:
   - #10 "Био сироп от липа" е САМО 750мл - НЕ 500мл, НЕ 250мл!
   - #11 "Био Пасирани домати" е САМО 680г - НЕ 350г, НЕ 400г!
   - #3 "Айран" е САМО 500мл
   - Ако грамажът/обемът не съвпада точно - НЕ СЪПОСТАВЯЙ!

2. ВНИМАВАЙ ЗА ПОДОБНИ ПРОДУКТИ:
   - "Сироп от ЛИПА" ≠ "Сироп от БЪЗ" ≠ "Сироп от МЕНТА" (различни вкусове!)
   - "Пасирани ДОМАТИ" ≠ "Домати КОНСЕРВА" ≠ "Кетчуп" 
   - "Вафла КЛАСИКА 40г" ≠ "Вафла БЕЗ ЗАХАР 40г" ≠ "Вафла 30г"
   - "Локум РОЗА" ≠ "Локум НАТУРАЛЕН"

3. ИЗПОЛЗВАЙ САМО ЦЕНИ ОТ СПИСЪКА "ПРОДУКТИ НАМЕРЕНИ В САЙТА"
   - НЕ измисляй цени!
   - Ако не намериш точно съвпадение - пропусни!

4. АКО НЕ СИ 100% СИГУРЕН - ПРОПУСНИ!

КОНКРЕТНИ СЪВПАДЕНИЯ (следвай точно):
- #1 "Био Локум роза 140г" = търси "локум" + "роза" + "140"
- #2 "Био Обикновени бисквити 150г" = търси "бисквити" + "краве масло" + "150" (НЕ с какао!)
- #3 "Айран harmonica 500мл" = търси "айран" + "500"
- #4 "Био Тунквана вафла без захар 40г" = търси "вафла" + "без захар" + "40"
- #5 "Био Оризови топчета черен шоколад 50г" = търси "оризови топчета" + "черен шоколад" + "50"
- #6 "Био лимонада 330мл" = търси "лимонада" + "330"
- #7 "Био тънки претцели 80г" = търси "претцели" + "80"
- #8 "Био тунквана вафла Класика 40г" = търси "вафла" + "класика" + "40"
- #9 "Био вафла без добавена захар 30г" = търси "вафла" + "30" (НЕ 40г!)
- #10 "Био сироп от липа 750мл" = търси "сироп" + "липа" + "750" (ЗАДЪЛЖИТЕЛНО 750мл!)
- #11 "Био Пасирани домати 680г" = търси "пасирани домати" + "680" (ЗАДЪЛЖИТЕЛНО 680г!)
- #12 "Smiles с нахут 50г" = търси "smiles" + "нахут" + "50"
- #13 "Био Крема сирене 125г" = търси "крема сирене" + "125"
- #14 "Козе сирене 200г" = търси "козе сирене" + "200"

ФОРМАТ: JSON обект с номера (1-14) като ключове и цени като стойности.
Пример: {{"3": 2.90, "8": 2.00, "10": 14.29}}

Ако нищо не съвпада със сигурност: {{}}"""
